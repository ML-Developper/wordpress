...
(function ($) {
                ...
                while (1) {
                    switch (_context.prev = _context.next) {
                        ...
                        case 8:
                            ...
                            wp.ajax.post('ecliptica_front_set_predesign', {
                                index: selectedFrontPage,
                                nonce: ecliptica_builder_status.ecliptica_front_set_predesign_nonce
                            });
                            if (ecliptica_builder_status.status === "not-installed") {
                                toggleProcessing(true);
                                installBuilder(callback);
                            }

                            if (ecliptica_builder_status.status === "installed") {
                                toggleProcessing(true);
    ...
    var $notice_container = $('.ecliptica-admin-big-notice--container');
    ...
    function disableNotice() {
        wp.ajax.post('ecliptica_disable_big_notice', {
            nonce: ecliptica_builder_status.ecliptica_disable_big_notice_nonce
        });
    }

    function toggleProcessing(value) {
        $(window).on('beforeunload.ecliptica-admin-big-notice', function () {
            return true;
        });
        if (value) {
            $('.ecliptica-admin-big-notice').addClass('processing');
            $('.ecliptica-admin-big-notice .action-buttons').fadeOut();
        } else {
            $('.ecliptica-admin-big-notice').removeClass('processing');
        }
    }
    ...
    function installBuilder(callback) {
        pluginNotice(ecliptica_builder_status.messages.installing);
        $.get(ecliptica_builder_status.install_url).always(function () {
            toggleProcessing(true);
            activateBuilder(callback);
        }).always(function () {
            $(window).off('beforeunload.ecliptica-admin-big-notice');
        });
    }

    function activateBuilder(callback) {
        pluginNotice(ecliptica_builder_status.messages.activating);

        wp.ajax.post('ecliptica_activate_plugin', {
            slug: ecliptica_builder_status.slug,
            source: 'notice-' + selectedFrontPage,
            _wpnonce: ecliptica_builder_status.colibri_plugin_install_activate_nonce
        }).done(function () {
            $(window).off('beforeunload.ecliptica-admin-big-notice');
            if (callback) {
                callback();
    ...
    $notice_container.on('click', '.view-all-demos', function () {
        ...
        processBuilderInstalationStepts(function () {
            window.location = ecliptica_builder_status.view_demos_url;
        });
    });
    ...
    var eclipticaInstallPluginSuccess = function eclipticaInstallPluginSuccess(response) {
    ...
    var eclipticaInstallPlugin = function eclipticaInstallPlugin(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: eclipticaInstallPluginSuccess
        });
    };

    var eclipticaActivatePlugin = function eclipticaActivatePlugin(event) {
        var $button = $(event.target);
    ...
    $document.on("click", ".colibri-plugin.install-now", eclipticaInstallPlugin);
    $document.on("click", ".colibri-plugin.activate-now", eclipticaActivatePlugin);
