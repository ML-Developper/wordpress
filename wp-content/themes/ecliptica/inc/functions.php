...
function ecliptica_page_builder_components($components)
{
...
function ecliptica_default_components($components)
{
...
function ecliptica_register_components($components = array())
{
    if (apply_filters('colibri_page_builder/installed', false)) {
        $components = ecliptica_page_builder_components($components);
    } else {
        $components = ecliptica_default_components($components);
    }

    return $components;
}

Hooks::prefixed_add_action('components', 'ecliptica_register_components');
Theme::load(array(
    'themeRelativePath' 		=> '',
 ...
 */
function ecliptica_assets()
{
    return ecliptica_theme()->getAssetsManager();
}
...
add_action('after_setup_theme', function() {
    ecliptica_theme()->register_menus(array(
        'header-menu' => esc_html__('Header Menu', 'ecliptica'),
        'footer-menu' => esc_html__('Footer Menu', 'ecliptica'),
    ));
...
add_action('widgets_init', 'ecliptica_register_sidebars');
function ecliptica_register_sidebars()
{
    register_sidebar(array(
        'name' 			=> esc_html__('Blog sidebar widget area', 'ecliptica'),
    ...
    if (function_exists('\is_shop')) {
        register_sidebar(array(
            'name' 			=> esc_html__('Woo Commerce left sidebar widget area', 'ecliptica'),
            'id' 			=> 'ecliptica-ecommerce-left',
            'before_widget' => '<div id="%1$s" class="widget %2$s">',
            'before_title' 	=> '<h5 class="widgettitle">',
            'after_title' 	=> '</h5>',
            'after_widget' 	=> '</div>',
    ...
    if (!apply_filters('colibri_page_builder/installed', false)) {
        ecliptica_assets()
            ...
            ->registerStylesheet("ecliptica-theme", "/theme/theme.css")
    ...
    ecliptica_assets()->registerTemplateStyle('ecliptica-theme-extras', '/theme/extras.css', 'ecliptica-theme');
...
//blog options
    ...
    function ecliptica_show_post_meta_setting_filter($value)
    {
    ...
    add_filter('colibri_show_post_meta', 'ecliptica_show_post_meta_setting_filter');


    function ecliptica_posts_per_row_setting_filter($value)
    {
    ...
    add_filter('colibri_posts_per_row', 'ecliptica_posts_per_row_setting_filter');

    function ecliptica_archive_post_highlight_setting_filter($value)
    {
    ...
    add_filter('colibri_archive_post_highlight', 'ecliptica_archive_post_highlight_setting_filter');


    function ecliptica_blog_sidebar_enabled_setting_filter($value)
    {
    ...
    Hooks::prefixed_add_filter('blog_sidebar_enabled', 'ecliptica_blog_sidebar_enabled_setting_filter');
//add_filter( 'blog_sidebar_enabled', 'ecliptica_blog_sidebar_enabled_setting_filter' );

    function ecliptica_override_with_thumbnail_image($value)
    {
        global $post;
    ...
    add_filter('colibri_override_with_thumbnail_image', 'ecliptica_override_with_thumbnail_image');

    function ecliptica_print_archive_entry_class($class = "")
    {
        ...
        $postsPerRow = max(1, apply_filters('ecliptica_posts_per_row', $default));


        $classes[] = "h-col-sm-12 h-col-md-" . (12 / intval($postsPerRow));

        $classes = apply_filters('ecliptica_archive_entry_class', $classes);
    ...
    function ecliptica_print_masonry_col_class($echo = false)
    {

        global $wp_query;
        $index = $wp_query->current_post;
        $hasBigClass = (is_sticky() || ($index === 0 && apply_filters('ecliptica_archive_post_highlight', false)));
    ...
    Hooks::prefixed_add_filter('info_page_tabs', 'ecliptica_get_started_info_page_tab');

    function ecliptica_get_started_info_page_tab($tabs)
    {
    ...
    function ecliptica_theme_plugins($plugins)
    {
        $theme_plugins = array();
    ...
    Hooks::prefixed_add_filter('theme_plugins', 'ecliptica_theme_plugins');


    add_filter('http_request_host_is_external', 'ecliptica_allow_internal_host', 10, 3);
    function ecliptica_allow_internal_host($allow, $host, $url)
    {
    ...
    add_action('wp_ajax_ecliptica_front_set_predesign', function () {
        check_ajax_referer( 'ecliptica_front_set_predesign_nonce', 'nonce' );
    ...
    function ecliptica_override_main_row_class($classes)
    {
        return Defaults::get('templates.blog.row.layout-classes', $classes);
    }

    Hooks::prefixed_add_filter('main_row_class', 'ecliptica_override_main_row_class', 10, 1);
...
function ecliptica_has_fresh_site() {
	$mods = get_theme_mods();
